...
IUSE="dbus debug declarative designer doc examples gles2 gui help multimedia
	...
	testlib webchannel webkit webengine websockets widgets x11extras xmlpatterns"
...
DEPEND="${RDEPEND}
	webengine? ( 
		widgets? ( >=dev-qt/qtwebengine-${QT_PV}[widgets] )
		!widgets? ( >=dev-qt/qtwebengine-${QT_PV} )
	)
...
src_configure() {
	configuration() {
		local myconf=(
			...
			$(pyqt_use_enable webengine QtWebEngineCore QtWebEngineWidgets QtWebChannel)
